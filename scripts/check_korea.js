...
async function checkKorea() {
  ...
  const { data: jogos } = await supabase.from('jogos')
    ...
    .eq('time_a', 'South Korea')
    .eq('time_b', 'Czechia');

  if (jogos.length > 0) {
    const game = jogos[0];
    console.log(`Jogo: South Korea x Czechia | Placar Oficial: ${game.placar_oficial_a} x ${game.placar_oficial_b}`);
    ...
    console.log('Jogo South Korea x Czechia não encontrado.');
  }
}
